...
AT+CON[mac] -> Connect device by using mac address.
...
DECL_FLASH_STR2(Str_PIN,         "PIN");   // BT Password (PIN)
DECL_FLASH_STR2(Str_BAUD,        "BAUD");  // 1-1200,2-2400,3-4800,4-9600,5-19200,6-38400,7-57600,8-115200,9-230400,default4-9600
...
DECL_FLASH_STR2(Str_CONN,        "CONN");  // Connection to a specific remote
...
enum {AT_AT = 0, AT_STATE, AT_PIN, AT_BAUD, AT_SHOW, AT_RMAAD, AT_ROLE, AT_NAME, AT_RNAME, AT_CMODE,
      AT_DISC, AT_INQM, AT_INQ, AT_INQC, AT_CONN, AT_IPSCAN, AT_IAC, AT_RESET, AT_CMD_MAX_NB};

DECL_FLASH_TBL(AtCmdTbl, char * const) = {Str_AT, Str_STATE, Str_PIN, Str_BAUD, Str_SHOW, Str_RMAAD, Str_ROLE, Str_NAME,
Str_RNAME, Str_CMODE,  Str_DISC, Str_INQM, Str_INQ, Str_INQC, Str_CONN, Str_IPSCAN, Str_IAC, Str_RESET};
...
static void    resetSet(char* Addon);
...
DECL_FLASH_TBL(AtCmdBtInit, AtCmdSt_t) = {
                          ...
                          //{AT_RESET, BT_CMD, resetSet,    Str_CRLF,           0,    0,     BT_SET_TIMEOUT_MS},
...
void bluetooth_init()
{
 uint32_t RateTbl[] = {115200, 57600, 38400, 19200, 9600};//1-1200,2-2400,3-4800,4-9600,5-19200,6-38400,7-57600,8-115200,9-230400,default4-9600
 uint8_t  RateValTbl[] = {8, 7, 6, 5, 4};
 uint8_t  Idx;
 char     UartAtCmd[30];
 char     RespBuf[10];
...
int8_t bluetooth_getPswd(char *RespBuf, uint8_t RespBufMaxLen, uint16_t TimeoutMs)
{
  ...
  Ret = sendAtCmdAndWaitForResp(AT_PIN, BT_GET, NULL, RespBuf, RespBufMaxLen, 1, 5, Str_CRLF_OK_CRLF, TimeoutMs); // Check only if the answer starts with P (PIN" or PSWD expected !)
  if(Ret > 0)
  {
    if(RespBuf[Ret - 1] == '"')
    {
...
int8_t bluetooth_setPswd(char *BtPswd, uint16_t TimeoutMs)
{
  ...
  return(sendAtCmdAndWaitForResp(AT_PIN, BT_SET, CmdBtPswd, RespBuf, sizeof(RespBuf), 0, 0, Str_OK_CRLF, TimeoutMs));
}
