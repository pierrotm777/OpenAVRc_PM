...
static uint8_t build_rcdata_pkt()
{
  ...
  uint8_t crsfTxBufferCount = 25;
    ...
    for ( i = 0; i < 25; i++) {
      ...
      Usart0TxBuffer_p2M[--crsfTxBufferCount] = (i<<2) | ((pulse>>8)&0x03); // Encoded channel + upper 2 bits pulse width.
      Usart0TxBuffer_p2M[--crsfTxBufferCount] = pulse & 0xff; // Low byte
    }
