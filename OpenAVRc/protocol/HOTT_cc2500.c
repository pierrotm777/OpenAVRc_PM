...
#define HOTT_BIND rfState8_p2M
#define HOTT_TELEMETRY (g_model.rfOptionBool1)
...
// SYNC1    D3
// SYNC0    91
// PKTLEN   32 - packet_p2M length, 50 bytes
// PKTCTRL1 04 - APPEND_STATUS on=RSSI+LQI, all other are receive parameters - irrelevant
// PKTCTRL0 44 - whitening, use FIFO, use CRC, fixed packet_p2M length
// ADDR     00
// CHANNR   10
// FSCTRL1  09 - IF
...
// FOCCFG   1D
const PROGMEM uint8_t ZZ_HOTTInitSequence[] = {
...
static void HOTT_rf_init()
{
  ...
  for (uint8_t i = 0; i<(DIM(ZZ_HOTTInitSequence)); ++i)
  {
...
{
	packet_p2M[0] = pgm_read_word_near( &HOTT_hop_val[num_ch] );
	...
	if(HOTT_BIND)//IS_BIND_IN_PROGRESS
	{
		memset(&packet_p2M[40],0xFA,5);
		memcpy(&packet_p2M[45],temp_rfid_addr_p2M,5);
	}
	else
	{
		memcpy(&packet_p2M[40],temp_rfid_addr_p2M,5);
		//????????????????
		/*uint16_t addr=HOTT_EEPROM_OFFSET+RX_num*5;
		//debug("RXID: ");
		for(uint8_t i=0;i<5;i++)
		{
			packet_p2M[45+i]=eeprom_read_byte((EE_ADDR)(addr+i));
			//debug(" %02X",packet_p2M[45+i]);
		}*/
		//debugln("");
		//????????????????
	}
...
static void HOTT_prep_data_packet()
{
	...
	packet_p2M[2] = channel_index_p2M;
	...
	for(uint8_t i=4;i<28;i+=2)
	{
		...
		val=FULL_CHANNEL_OUTPUTS(ch);//val=Channel_data[ch];
		val=(((val<<2)+val)>>2)+860*2;					// value range 860<->2140 *2 <-> -125%<->+125%
		#ifdef FAILSAFE_ENABLE
			if(failsafe_count==1)
			{ // first failsafe packet_p2M
				packet_p2M[3] |= 0x40;
			...
			{ // second failsafe packet_p2M=timing?
				packet_p2M[3] |= 0x50;
				if(i==4)
					val=2;
				else
	...
	packet_p2M[29] = 0x02;									// 0x02 when bind starts then when RX replies cycle in sequence 0x1A/22/2A/0A/12, 0x02 during normal packet_p2Ms, 0x01->text config menu, 0x0A->no more RX telemetry
		...
		if(!HOTT_BIND)//if(IS_BIND_DONE)
		{
			...
			{//Text mode
				...
				if(g_model.rfSubType == 0)//==HOTT_SYNC
	...
	switch(send_seq_p2M)
	{
      ...
      SCHEDULE_MIXER_END_IN_US(HOTT_PACKET_PERIOD); // Schedule next Mixer calculations.
		...
		case HOTT_DATA2:
			...
			{ //Channel is busy
				//systemBolls.rangeModeIsOn = true; // manage low power TX//LBT_POWER_on;										// Reduce to low power before transmitting
        ...
        uint8_t * rxBuf = &pulses2MHz.pbyte[CHANNEL_USED_OFFSET + HOTT_NUM_RF_CHANNELS]; //Use 50 Channel MAX -> 29 free bytes
				CC2500_ReadData(rxBuf, len);
				if((rxBuf[HOTT_RX_PACKET_LEN+1]&0x80) && memcmp(temp_rfid_addr_p2M,rxBuf,5)==0)
				{ // CRC OK and TX ID matches
					if(HOTT_BIND)//IS_BIND_IN_PROGRESS
					{
						...
						*/
						//uint16_t addr=HOTT_EEPROM_OFFSET+RX_num*5;
						//for(uint8_t i=0; i<5; i++)
						//	eeprom_write_byte((EE_ADDR)(addr+i),rxBuf[5+i]);
						...
						{	//Telemetry
							...
							//   Page 0 [18,19] = [19]<<8+[18]=max lost packet_p2M time in ms, max value seems 2s=0x7D0
							//   Page 0 [20] = 0x00 ??
							//
							...
							//   Menu commands are sent through TX packet_p2Ms:
							...
							rxBuf[0]= rxBuf[HOTT_RX_PACKET_LEN];
							rxBuf[1]= TX_LQI;
							...
							{ //Text mode
								...
								rxBuf[10] = 0x80;									// Marking telem Text mode
								rxBuf[12] = 0;
								for(uint8_t i=0; i<HOTT_SENSOR_TYPE;i++)
									rxBuf[12] |= HOTT_sensor_ok[i]<<i;				// Send detected sensors
							}
							else
							{ //Binary sensor
								HOTT_sensor_seq %= 5;									// 5 pages in binary mode per sensor
								if(Hott_State==0 && HOTT_sensor_ok[0]==false && HOTT_sensor_ok[1]==false && HOTT_sensor_ok[2]==false && HOTT_sensor_ok[3]==false && HOTT_sensor_ok[4]==false && HOTT_sensor_ok[5]==false)
									HOTT_sensor_seq=0;									// No sensors always ask page 0
								if(Hott_State)
									Hott_State--;
								if( rxBuf[11]==1 )									// Page 1
								{
									if( rxBuf[12] == (HOTT_sensor_cur+9)<<4 )
									{ //Requested sensor is sending: 0x90/A0/B0/C0/D0/E0
										...
										HOTT_sensor_ok[(rxBuf[12]>>4)-9]=true;
									}
									else
									{
										HOTT_sensor_valid = false;
								...
								{ //Page != 0
									...
									{
										rxBuf[10] = HOTT_sensor_cur+9;				// Mark telem with sensor ID
										HOTT_sensor_pages |= 1<<rxBuf[11];			// Page received
									}
									else
										send_telem=false;								// Do not send
								}
								else
									rxBuf[10]=0;									// Mark telem with sensor 0=RX
							}
							...
							{ //Next sensor
								...
								while(HOTT_sensor_ok[HOTT_sensor_cur]==false && loop<HOTT_SENSOR_TYPE+1 && Hott_State==0);
								HOTT_sensor_valid=false;
								HOTT_sensor_pages=0;
								HOTT_sensor_seq=0;
...
{
 uint16_t time = HOTT_callback();
 heartbeat |= HEART_TIMER_PULSES;
 CALCULATE_LAT_JIT(); // Calculate latency and jitter.
 return time;
}

static void HOTT_initialise(uint8_t bind)
{
  //num_ch=random(0xfefefefe)%16;
  srandom(g_eeGeneral.fixed_ID.ID_32 & 0xfefefefe);
  uint8_t idx = random()%16;
	num_ch=idx;
