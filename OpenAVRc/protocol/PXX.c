#include "PXX.h"

#define PPM_CENTER                          1500
#define PPM_LOW                             817
#define PPM_HIGH                            2182
#define PPM_HIGH_ADJUSTED                   (PPM_HIGH - PPM_LOW)
#define PXX_CHANNEL_WIDTH                   2048
#define PXX_UPPER_LOW                       2049
#define PXX_UPPER_HIGH                      4094
#define PXX_LOWER_LOW                       1
#define PXX_LOWER_HIGH                      2046

const uint16_t CRC_Short[] =
{
   0x0000, 0x1189, 0x2312, 0x329B, 0x4624, 0x57AD, 0x6536, 0x74BF,
   0x8C48, 0x9DC1, 0xAF5A, 0xBED3, 0xCA6C, 0xDBE5, 0xE97E, 0xF8F7
};

uint16_t CRCTable(uint8_t val)
{
	return CRC_Short[val&0x0F] ^ (0x1081 * (val>>4));
}


void PXX_crc( uint8_t data )
{
    pcmCrc=(pcmCrc<<8) ^ CRCTable((pcmCrc>>8)^data) ;
}

/*
void USART_Init(long baud)
{
  Serial.begin(125000);
  //  int _baud = (16000000 / (2 * baud)) - 1;

    UBRR0 = 0;

    // Setting the XCKn port pin as output, enables master mode.
    //XCKn_DDR |= (1<<XCKn);
    //DDRD = DDRD | B00000010;

    // Set MSPI mode of operation and SPI data mode 0.
    UCSR0C = (1<<UMSEL01)|(1<<UMSEL00)|(1<<UDORD0);

    // Enable receiver and transmitter.
    UCSR0B = (1<<TXEN0);
    // Set baud rate.
    // IMPORTANT: The Baud Rate must be set after the transmitter is enabled
    UBRR0 = _baud;

}
*/
/*
void USART_Send(uint8_t data) {
    // Put data into buffer, sends the data
    UDR0 = data;

    //Wait for the buffer to be empty
    while ( !( UCSR0A & (1<<UDRE0)) );
}

void PXX_begin()
{
    USART_Init(125000);
}
*/
void PXX_putPcmSerialBit(uint8_t bit)
{
    PXX_serialByte >>= 1;
    if (bit & 1)
    {
        PXX_serialByte |= 0x80;
    }

    if (++PXX_serialBitCount >= 8)
    {
        PXX_pulses[length++] = PXX_serialByte;
        PXX_serialBitCount = 0;
    }
}

// 8uS/bit 01 = 0, 001 = 1
void PXX_putPcmPart(uint8_t value)
{
    PXX_putPcmSerialBit(0);

    if (value)
    {
        PXX_putPcmSerialBit(0);
    }

    PXX_putPcmSerialBit(1);
}

void PXX_putPcmFlush()
{
    while (PXX_serialBitCount != 0)
    {
        PXX_putPcmSerialBit(1);
    }
}

void PXX_putPcmBit(uint8_t bit)
{
    if (bit)
    {
        PXX_pcmOnesCount += 1;
        PXX_putPcmPart(1);
    }
    else
    {
        PXX_pcmOnesCount = 0;
        PXX_putPcmPart(0);
    }

    if (PXX_pcmOnesCount >= 5)
    {
        PXX_putPcmBit(0);                                // Stuff a 0 bit in
    }
}

void PXX_putPcmByte(uint8_t byte)
{
    PXX_crc(byte);

    for (uint8_t i=0; i<8; i++)
    {
        PXX_putPcmBit(byte & 0x80);
        byte <<= 1;
    }
}

void PXX_putPcmHead()
{
    // send 7E, do not CRC
    // 01111110
    PXX_putPcmPart(0);
    PXX_putPcmPart(1);
    PXX_putPcmPart(1);
    PXX_putPcmPart(1);
    PXX_putPcmPart(1);
    PXX_putPcmPart(1);
    PXX_putPcmPart(1);
    PXX_putPcmPart(0);
}

void PXX_prepare(int16_t channels[16])
{
    uint16_t chan=0, chan_low=0;

    PXX_length = 0;
    PXX_pcmCrc = 0;
    PXX_pcmOnesCount = 0;

    /* Preamble */
    PXX_putPcmPart(0);
    PXX_putPcmPart(0);
    PXX_putPcmPart(0);
    PXX_putPcmPart(0);

    /* Sync */
    PXX_putPcmHead();

    // Rx Number
    PXX_putPcmByte(16);

    // FLAG1 - Fail Safe Mode, nothing currently set, maybe want to do this
    PXX_putPcmByte(0);

    // FLAG2
    PXX_putPcmByte(0);

    // PPM
    for (int i=0; i<8; i++)
    {

        int channelPPM = channels[(PXX_sendUpperChannels ? (8 + i) : i)];
        float convertedChan = ((float(channelPPM) - float(PPM_LOW)) / (float(PPM_HIGH_ADJUSTED))) * float(PXX_CHANNEL_WIDTH);
        chan = limit((PXX_sendUpperChannels ? PXX_UPPER_LOW : PXX_LOWER_LOW),
                     convertedChan,
                     (PXX_sendUpperChannels ? PXX_UPPER_HIGH : PXX_LOWER_HIGH));

        if (i & 1)
        {
            PXX_putPcmByte(chan_low); // Low byte of channel
            PXX_putPcmByte(((chan_low >> 8) & 0x0F) | (chan << 4));  // 4 bits each from 2 channels
            PXX_putPcmByte(chan >> 4);  // High byte of channel
        }
        else
        {
            chan_low = chan;
        }
    }

    // CRC16
    PXX_putPcmByte(0);
    chan = PXX_pcmCrc;
    PXX_putPcmByte(chan>>8);
    PXX_putPcmByte(chan);

    // Sync
    PXX_putPcmHead();
    PXX_putPcmFlush();
}

void PXX_send()
{
    for(int i = 0; i < PXX_length; i++)
    {
        Serial.print(PXX_pulses[i]);//USART_Send(pulses[i]);
    }

    PXX_sendUpperChannels = !PXX_sendUpperChannels;
}

uint16_t PXX_limit(uint16_t low, uint16_t val, uint16_t high) {
    if(val < low)
    {
        return low;
    }

    if(val > high)
    {
        return high;
    }

    return val;
}

