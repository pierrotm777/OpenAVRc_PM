...
#include <avr/wdt.h>
  ...
  uint8_t current_power = tx_power_low;
  uint8_t power_delay_counter = TX_POWER_DELAY_FILTER;
 ...
 */
  ...
  switch (stateMachine) {
    ...
    case RECEIVE:     // stay in RECEIVE and wait for data until next TX period
      if (receiveData(6)) {
        no_RX_ack = 0;
        TX_RSSI = RX_Buffer[0] - 255;// - 255; "0" to display uint8_t values instead of dBm
        #ifdef DEBUG_CH_FREQ
        Serial.print("\tHC: ");
  ...
  if (micros() > transmit_time) {
      ...
      Serial.print(uint8_t((micros() - packet_timer)/1000)); Serial.print("\t");
